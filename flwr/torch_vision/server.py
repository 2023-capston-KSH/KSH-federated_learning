...
def get_evaluate_fn(model: torch.nn.Module, toy: bool, valset: torch.utils.data.Subset, save_path: str):
...
def load_data(toy: bool, data: str, data_dir: str) -> Tuple[torch.utils.data.Dataset, torch.utils.data.Dataset]:
	...
	if toy:
		...
		valset = torch.utils.data.Subset(
			trainset, range(n_train - 10, n_train))
	else:
		# Use the last 5k training examples as a validation set
		valset = torch.utils.data.Subset(
			trainset, range(n_train - 5000, n_train))
...
def create_strategy(
	...
	return fl.server.strategy.FedAvg(
		...
		on_fit_config_fn=lambda r: fit_config(
			r, args.batch_size, args.local_epochs),
